...
int main (int argc, char *argv[]) {
  ...
  double Ttot = 0.0, T = 0.0;
	double totalMom = 0.0, localTotalMom = 0.0;
  int sent = 0, sentTot = 0;
	cord_t wall;
  ...
  for (int t = 0; t < MAX_TIME; ++t) {
    ...
    while(i < numParticles) {
      ...
      for (int k = i + 1; k < particles.size(); ++k) {
        ...
        if (collision >= 0) {
          ...
          localTotalMom += wall_collide(&particles[k].pcord, wall);
          continue;
        }
      }
      ...
      newTarget = target(AREA, particles[i].pcord);
      if(newTarget != myid) {
    ...
    //printf("Process # %d send %d particles, t=%d\n", myid, sentParticles, t);
    printf("%d\n", sentParticles);
  ...
  MPI_Reduce(&T, &Ttot, numProc, MPI_DOUBLE, MPI_SUM, root, MPI_COMM_WORLD);
  MPI_Reduce(&sent, &sentTot, numProc, MPI_INT, MPI_SUM, root, MPI_COMM_WORLD);
  VT_leave(VT_NOSCL);
    ...
    printf("Temperature is %g\n", Ttot/numProc);
    printf("Sent particles: %d\n", sentTot);
	}
